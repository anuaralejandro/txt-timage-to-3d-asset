...
print("Initializing VisualBruno ComfyUI-Trellis2 MultiView AAA Quality Test...", flush=True)
...
print("Step 3: Generating 3D Mesh with Voxel (8GB Optimized, MultiView, AAA Quality)...")
...
mesh_output = generator.process(
    ...
    sparse_structure_steps=50,  # Increased from 12 for AAA quality
    shape_steps=50,             # Increased from 12
    texture_steps=50,           # Increased from 12
    max_num_tokens=49152,
    sparse_structure_resolution=64, # Increased for finer details and to avoid holes
    generate_texture_slat=True,
    use_tiled_decoder=True,
    sampler="rk4",              # RK4 is slower but provides much higher quality than euler
    fill_holes=True,
    hole_iterations=3,          # Increased to aggressively fix bullet holes
    hole_fill_algorithm="flood_fill",
    keep_only_shell=True,
    back_image=back,
    left_image=left,
    right_image=right,
    sparse_structure_guidance_strength=7.5, # Slightly higher guidance
...
glb_res = exporter.process(mesh, resolution=1024, texture_size=4096, target_face_num=2000000)[0]

# Write out to disk properly using trimesh export
output_path = r"C:\Users\datam\Downloads\jace0_limpios\jace0_3d_asset_hq.glb"
glb_res.export(output_path)
print(f"-> Exported to: {output_path}")

print("\n=======================================================")
print(f"🎉 VISUALBRUNO TRELLIS2 MULTIVIEW AAA 3D GENERATION SUCCESSFUL!")
