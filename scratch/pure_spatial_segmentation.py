...
for i in range(num_faces):
    ...
    # 1. Head and Hair (Y > 0.42)
    if y > 0.42:
        labels[i] = SemanticLabel.HAIR if (y > 0.62 or z < -0.05) else SemanticLabel.HEAD

    # 2. Neck (y in [0.38, 0.42], abs(x) < 0.15)
    elif 0.38 < y <= 0.42 and abs(x) < 0.15:
        labels[i] = SemanticLabel.NECK

    # 3. Arms (Y > 0.05, abs(X) > 0.16)
    elif y > 0.05 and abs(x) > 0.16:
        if abs(x) > 0.38:
    ...
    # 6. Torso (Y in [-0.05, 0.38], central X)
    else:
...
# Smooth boundaries cleanly without creating joint rings
