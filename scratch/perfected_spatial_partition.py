...
for i in range(num_faces):
    ...
    # 2. Neck (y in [0.45, 0.58], abs(x) <= 0.16)
    elif 0.45 < y <= 0.58 and abs(x) <= 0.16:
        labels[i] = SemanticLabel.NECK

    # 3. T-Pose Arms (y in [0.0, 0.48], abs(x) > 0.16)
    elif 0.0 <= y <= 0.48 and abs(x) > 0.16:
        if abs(x) > 0.45:
    ...
    # 5. Upper Legs / Thighs (Y in [-0.42, 0.0])
    elif -0.42 < y <= 0.0:
        labels[i] = SemanticLabel.LEG_UPPER_L if x < 0 else SemanticLabel.LEG_UPPER_R

    # 6. Torso (Everything else in central body)
    else:
...
# Postprocessing: smooth boundaries cleanly without joint rings
...
print("Exact 13-Class Breakdown (Before Postprocessing):")
